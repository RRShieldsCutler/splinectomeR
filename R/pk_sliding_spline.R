...
sliding_spliner <- function(data = NA, xvar = NA, yvar = NA, category = NA,
                            ...
                            cut_low = NA, ints = 100, quiet = FALSE) {
  ...
  cases.notsparse = cases.tab[cases.tab$Freq >= unit_number, ]
  ...
  df.groups = df %>% distinct_(cases, .keep_all = T)
  df.groups = df.groups %>% select_(cases, category)
  ...
  colnames(pval.df) = c(xvar, 'p-value', 'number_of_observations')
  result <- list('pval_table' = pval.df, 'spline_table' = spl.table)
  return(result)
}

#' @title Sliding spliner plot splines
...
#' 
  ...
  # Plot the splines
sliding_spliner.plot.splines <- function(data_df, cases = cases,
                                         xvar = xvar, yvar = yvar) {
  require(ggplot2)
  spl.p <- data_df
  plot.spline.data = melt(data = spl.p, id.vars = 'x')
...
sliding_spliner.plot.pvals <- function(data_df, cases = cases,
                                       xvar = xvar) {
  require(ggplot2)
  pval.p <- data_df
  .norm_range <- function(x){(x-min(x)) / (max(x)-min(x))}
  pval.p$N.norm = norm_range(x = pval.p$N)
  ggplot(pval.p, aes(x=x.series, y=p.value)) + geom_line() +
